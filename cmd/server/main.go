...
var startCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// 没设置参数则使用配置文件中的
		if listenAddress == "" {
			listenAddress = common.Config.Server.System.Listen
		}

		// 没设置参数则使用配置文件中的
		if listenPort == "" {
			listenPort = common.Config.Server.System.Port
		}

		// Logo
		fmt.Println(common.SERVER_LOGO)
